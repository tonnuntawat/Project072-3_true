Public Class frmMain
    Private Sub PictureBox1_Click(sender As Object, e As EventArgs) Handles PictureBox1.Click

    End Sub

    Private Sub ประเภทสนคาToolStripMenuItem_Click(sender As Object, e As EventArgs)

    End Sub

    Private Sub เกยวกบระบบToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles เกยวกบระบบToolStripMenuItem.Click

    End Sub

    Private Sub Lab1ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles Lab1ToolStripMenuItem.Click

    End Sub

    Private Sub Lab2ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles Lab2ToolStripMenuItem.Click

    End Sub

    Private Sub Lab3ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles Lab3ToolStripMenuItem.Click

    End Sub

    Private Sub Lab4ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles Lab4ToolStripMenuItem.Click


    End Sub

    Private Sub Lab5ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles Lab5ToolStripMenuItem.Click


    End Sub

    Private Sub Lab6ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles Lab6ToolStripMenuItem.Click


    End Sub

    Private Sub Lab7ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles Lab7ToolStripMenuItem.Click


    End Sub

    Private Sub Lab8ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles Lab8ToolStripMenuItem.Click


    End Sub

    Private Sub ใบงาน1ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ใบงาน1ToolStripMenuItem.Click
        frmLab1.ShowDialog()
    End Sub

    Private Sub ใบงาน2ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ใบงาน2ToolStripMenuItem.Click
        frmLab2.ShowDialog()
    End Sub

    Private Sub ใบงาน3ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ใบงาน3ToolStripMenuItem.Click
        frmLab3.ShowDialog()
    End Sub

    Private Sub ใบงาน4ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ใบงาน4ToolStripMenuItem.Click
        frmJob4.ShowDialog()
    End Sub

    Private Sub ใบงาน5ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ใบงาน5ToolStripMenuItem.Click
        frmLab5.ShowDialog()
    End Sub

    Private Sub ใบงาน6ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ใบงาน6ToolStripMenuItem.Click
        frmLab6.ShowDialog()
    End Sub

    Private Sub ใบงาน7ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ใบงาน7ToolStripMenuItem.Click
        frmLab7.ShowDialog()
    End Sub

    Private Sub ใบงาน8ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ใบงาน8ToolStripMenuItem.Click
        frmLab8.ShowDialog()
    End Sub

    Private Sub ใบงาน9ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ใบงาน9ToolStripMenuItem.Click
        frmLab10.ShowDialog()
    End Sub

    Private Sub ใบงาน10ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ใบงาน10ToolStripMenuItem.Click
        frmLab11.ShowDialog()
    End Sub

    Private Sub ใบงาน11ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ใบงาน11ToolStripMenuItem.Click
        frmLab12.ShowDialog()
    End Sub
End Class